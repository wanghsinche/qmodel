...
export async function getBacktestingData(index = 'nasdaq100') {
    ...
    try {
        ...
        await kv.put(`${index}-myorders.csv::${today}`, JSON.stringify(data.find(el=>el.name === `${index}-myorders.csv`)?.csv), { expirationTtl: 60 * 60 * 24  })
        await kv.put(`${index}-mystats.csv::${today}`, JSON.stringify(data.find(el=>el.name === `${index}-mystats.csv`)?.csv), { expirationTtl: 60 * 60 * 24  })
        await kv.put(`${index}-other_stats.csv::${today}`, JSON.stringify(data.find(el=>el.name === `${index}-other_stats.csv`)?.csv), { expirationTtl: 60 * 60 * 24  })
    }
